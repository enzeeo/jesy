...
from disaster.app.routes import sim as sim_routes
...
def test_start_payload_seed_defaults_to_randomized_runs(monkeypatch):
    seeds = iter([101, 202])
    monkeypatch.setattr(sim_routes.secrets, "randbits", lambda _bits: next(seeds))

    assert sim_routes.StartPayload().seed is None
    assert sim_routes._effective_seed(None) == 101
    assert sim_routes._effective_seed(None) == 202
    assert sim_routes._effective_seed(42) == 42
